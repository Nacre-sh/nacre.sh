---
title: Documentation - Version Alpha
---
...
Bultins are commands available natively in Nacre. They don't need to be imported and can be used right away.
...
```js
cat.truncated(filepath, string): string
```

Write `string` content in the `filepath` file. The file is created (if it does not exist) or truncated (if it exists).
