...
Nacre relies on NodeJS so you must have it installed on your system. Follow their [installation instructions](https://nodejs.org/). Once this is done, install it using:
...
A prompt will invite you to run commands interactively. Try listing the content of the working directory by running `ls()`:

```js
Nacre (Node.js 16.13.1 - V8 9.4.146.24-node.14)
> ls()
[
  '.git',
  '.gitignore',
...
You may notice that `ls()` command, unlike `ls` on more traditional shells, requires parentheses to display items in the current working directory. The reason is simple, Nacre is empowered by NodeJS, which means everything you write must follow the JavaScript syntaxe.
...
This is particulary handy to understand what you can do with a command. In this case, it tells us that `ls` has a `recursive()` function.
