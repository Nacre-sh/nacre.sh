...
Nacre relies on **NodeJS version 17** so you must have it installed on your system. Follow their [installation instructions](https://nodejs.org/). Once this is done, install it using:
...
```js
Welcome to Nacre version Alpha. Find help at https://nacre.sh.
> ls()
[
  '.git',
  '.gitignore',
...
You may notice that `ls()` command, unlike `ls` on more traditional shells, requires parentheses to display items in the current working directory. The reason is simple, Nacre is empowered by NodeJS, which means everything you write must follow the [JavaScript syntaxe](https://developer.mozilla.org/en-US/docs/Web/JavaScript).
